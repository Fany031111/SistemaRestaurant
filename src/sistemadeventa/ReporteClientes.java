 ...
 * @author Fanny
 */
public class ReporteClientes extends javax.swing.JFrame implements Runnable{
    ...
    public ReporteClientes() {
        ...
        this.setTitle("Reporte de Productos - SISTEMA DE VENTAS");
        ...
        try {
            ...
            ConsultarDatos(fid.getText(), fnombre.getText(), fape.getText(), fcedula.getText());
    ...
    static void ConsultarDatos(String id, String nombre, String ape, String cedula) throws SQLException {
        ...
        try {
            ...
            StringBuilder query = new StringBuilder("SELECT * FROM tb_cliente WHERE 1=1");
            List<Object> params = new ArrayList<>();

            // Agregar filtros según los parámetros
            if (!id.equals("")) {
                query.append(" AND idcliente LIKE ?");
                params.add(id + "%");
            }
            if (!nombre.equals("")) {
            ...
            while (respuesta.next()) {
                ...
                cliente[4] = respuesta.getString(5);
                cliente[5] = respuesta.getString(6);
    ...
    private void initComponents() {
        ...
        jButton1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagenes/pdf stock.png"))); // NOI18N
        ...
        getContentPane().add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(520, 560, 150, 40));
        ...
        tclientes.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Nombre(s)", "Apellidos", "Cédula", "Teléfono", "Dirección"
            }
        ) {
        ...
        if (tclientes.getColumnModel().getColumnCount() > 0) {
            tclientes.getColumnModel().getColumn(0).setMinWidth(70);
            tclientes.getColumnModel().getColumn(0).setPreferredWidth(70);
            tclientes.getColumnModel().getColumn(0).setMaxWidth(70);
            ...
            tclientes.getColumnModel().getColumn(4).setMinWidth(120);
            tclientes.getColumnModel().getColumn(4).setPreferredWidth(120);
            tclientes.getColumnModel().getColumn(4).setMaxWidth(120);
        }

        getContentPane().add(scroll, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 180, 850, 370));

        jPanel1.setBackground(new java.awt.Color(0, 153, 204));
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Filtrar cliente por:", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 1, 18), new java.awt.Color(255, 255, 255))); // NOI18N
        ...
        jPanel1.add(fid, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 30, 60, 30));
        ...
        jPanel1.add(fape, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 30, 160, 30));
        ...
        jPanel1.add(jLabel16, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 30, -1, 30));

        fcedula.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        ...
        fcedula.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                fcedulaKeyReleased(evt);
            }
        });
        jPanel1.add(fcedula, new org.netbeans.lib.awtextra.AbsoluteConstraints(640, 30, 140, 30));

        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 90, 850, 80));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            // Verifica si el archivo tiene extensión ".pdf", si no la tiene, se la agrega
            ...
            try {
                ...
                try {
                    // Cargar el logo de la empresa
                    com.itextpdf.text.Image logo = com.itextpdf.text.Image.getInstance(getClass().getResource("/Imagenes/logoFINAL.png"));

                    // Ajusta la posición absoluta del logo (esquina superior derecha)
                    logo.setAbsolutePosition(440, 715); // Ajusta las coordenadas X, Y para ponerlo en la posición deseada (ajusta estos valores según sea necesario)
                    logo.scaleToFit(200, 100); // Ajustar el tamaño del logo (ancho x alto)

                    // Añadir el logo al documento (esto estará "por detrás" del texto)
                    documento.add(logo);
                ...
                // Para título grande
                Font f = new Font();
                f.setFamily(Font.FontFamily.HELVETICA.name());
                f.setStyle(Font.BOLD);
                f.setSize(20);
                // Para título
                Font f3 = new Font();
                f3.setFamily(Font.FontFamily.HELVETICA.name());
                f3.setStyle(Font.BOLD);
                f3.setSize(18);

                // Para subtítulo
                Font f2 = new Font();
                f2.setFamily(Font.FontFamily.HELVETICA.name());
                ...
                titulo.setFont(f);
                titulo.add("Restaurante 'Río Tonalá'");
                ...
                titulo2.setFont(f3);
                titulo2.add("Reporte de clientes");
                ...
                // Agregar la fecha y hora como pie de página, alineados a la derecha
                Paragraph fechaHora = new Paragraph();
                fechaHora.setAlignment(Element.ALIGN_CENTER);
                fechaHora.setFont(f2);
                fechaHora.add("Fecha: " + fechalbl.getText() + "   Hora: " + horalbl.getText());
                ...
                Font fontTabla = new Font();
                fontTabla.setFamily(Font.FontFamily.HELVETICA.name());
                fontTabla.setSize(8); // Tamaño de fuente reducido

                // Crear la tabla con las columnas seleccionadas
                PdfPTable tabla = new PdfPTable(6);
                float[] columnWidths = {0.5f, 2f, 2f, 1f, 1.2f, 3f};
                tabla.setWidths(columnWidths);

                // Agregar encabezados
                PdfPCell header = new PdfPCell(new Phrase("ID", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                header = new PdfPCell(new Phrase("Nombre(s)", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                header = new PdfPCell(new Phrase("Apellidos", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                header = new PdfPCell(new Phrase("Cédula", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                header = new PdfPCell(new Phrase("Teléfono", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                header = new PdfPCell(new Phrase("Dirección", fontTabla));
                header.setBackgroundColor(BaseColor.LIGHT_GRAY);
                tabla.addCell(header);

                // Agregar las filas seleccionadas desde el modelo `modelostock`
                for (int i = 0; i < modelo.getRowCount(); i++) {
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 0) == null ? "" : modelo.getValueAt(i, 0).toString()), fontTabla)); // Columna 1
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 1) == null ? "" : modelo.getValueAt(i, 1).toString()), fontTabla)); // Columna 2
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 2) == null ? "" : modelo.getValueAt(i, 2).toString()), fontTabla)); // Columna 4
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 3) == null ? "" : modelo.getValueAt(i, 3).toString()), fontTabla)); // Columna 5
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 4) == null ? "" : modelo.getValueAt(i, 4).toString()), fontTabla)); // Columna 5
                    tabla.addCell(new Phrase((modelo.getValueAt(i, 5) == null ? "" : modelo.getValueAt(i, 5).toString()), fontTabla)); // Columna 5
                ...
                // Cerrar el documento
                documento.close();
                JOptionPane.showMessageDialog(null, "Reporte creado en: " + archivo.getAbsolutePath());
    ...
    private void fcedulaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_fcedulaKeyReleased
        // TODO add your handling code here:
        try {
            limpiarDatosTabla();
            ConsultarDatos(fid.getText(), fnombre.getText(), fape.getText(), fcedula.getText());
    ...
    }//GEN-LAST:event_fcedulaKeyReleased
